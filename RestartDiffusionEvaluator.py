...
class RestartDiffusionEvaluator:
    ...
    @torch.no_grad()
    def generate(self, prompt_ids: list, steps_budget: int, is_hidden: bool = True) -> str:
    ...
    def process_chunk(self, prompt_ids: list, steps_budget: int = 6) -> str:
        return self.generate(prompt_ids, steps_budget, is_hidden=True)
    
    def finalize(self, prompt_ids: list, cleanup_steps: int = 24) -> str:
        result = self.generate(prompt_ids, cleanup_steps, is_hidden=False)
        self.last_output = result
        return result
    
    def run_full_offline_inference(self, prompt_ids: list, total_steps: int = 16) -> tuple:
        result = self.generate(prompt_ids, total_steps, is_hidden=False)
        self.last_output = result
        return result, None
